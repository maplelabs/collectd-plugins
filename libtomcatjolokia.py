...
def synchronized(func):
    """Synchronise to prevent race condition which can occur
       when  2  plugin try to start jolokia with same process."""
...
class JolokiaClient(object):
    ...
    def get_jolokia_port(self, pid):
        ...
        if err or ret:
            ...
            if err or ret:
                collectd.error("Plugin tomcat: Failed to start jolokia. Retrying again for pid %s" % pid)
                ...
                status, err, ret = self.run_jolokia_cmd("status", pid)
                if err or ret:
                    collectd.error("Plugin tomcat: Failed to start jolokia for pid %s" % pid)
                    return False
            else:
